...
st.subheader("ระบุชื่อหัวหน้าชุดตรวจค้น")
leader_names = [l["name"] for l in leaders] if 'leaders' in locals() else []
sign_choices = leader_names + all_officer_displays + ["อื่นๆ (ระบุ)"]

img_sign_opt = st.selectbox("ระบุชื่อหัวหน้าชุดตรวจค้น", sign_choices)
if img_sign_opt == "อื่นๆ (ระบุ)":
    img_signer = st.text_input("ระบุชื่อหัวหน้าชุดตรวจค้น")
else:
